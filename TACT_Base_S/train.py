...
def main(params):
    ...
    rel_feats = list()
    for idx in sorted(list(train.id2relation.keys())):
        feat = rel_external_embeds[train.id2relation[idx]]
    ...
    graph_classifier = initialize_model(params, dgl_model, rel_vectors, load_model=False)

    print(f"Device: {params.device}")
    print(f"Input dim : {params.inp_dim}, # Relations : {params.num_rels}, # Augmented relations : {params.aug_num_rels}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_epochs", "-ne", type=int, default=50, help="Learning rate of the optimizer")
