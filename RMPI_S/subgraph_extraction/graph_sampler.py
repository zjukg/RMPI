...
def get_average_subgraph_size(sample_size, links, A, params):
    ...
    for (n1, n2, r_label) in links[np.random.choice(len(links), sample_size)]:
        en_nodes, en_n_labels, dis_nodes, dis_n_labels = subgraph_extraction_labeling((n1, n2), r_label, A, params.hop, params.enclosing_sub_graph, params.max_nodes_per_hop)
...
def extract_save_subgraph(args_):
    ...
    en_nodes, en_n_labels, dis_nodes, dis_n_labels = subgraph_extraction_labeling((n1, n2), r_label, A_, params_.hop, params_.enclosing_sub_graph, params_.max_nodes_per_hop)
    ...
    if max_label_value_ is not None:
        en_n_labels = np.array([np.minimum(label, max_label_value_).tolist() for label in en_n_labels])
        dis_n_labels = np.array([np.minimum(label, max_label_value_).tolist() for label in dis_n_labels])

    datum = {'en_nodes': en_nodes, 'r_label': r_label, 'g_label': g_label, 'en_n_labels': en_n_labels, 'dis_nodes':dis_nodes, 'dis_n_labels':dis_n_labels}
...
def subgraph_extraction_labeling(ind, rel, A_list, h=1, enclosing_sub_graph=False, max_nodes_per_hop=None, max_node_label_value=None):
    ...
    enclosing_labels, enclosing_subgraph_nodes_labeled = node_label(incidence_matrix(enclosing_subgraph), max_distance=h, enclosing_flag=True)
    ...
    if max_node_label_value is not None:
        pruned_enclosing_labels = np.array([np.minimum(label, max_node_label_value).tolist() for label in pruned_enclosing_labels])
