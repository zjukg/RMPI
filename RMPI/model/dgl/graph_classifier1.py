...
class GraphClassifier(nn.Module):
    def __init__(self, params, relation2id):  # in_dim, h_dim, rel_emb_dim, out_dim, num_rels, num_bases):
        ...
        self.fc_reld3 = nn.ModuleList([nn.Linear(self.params.rel_emb_dim, self.params.rel_emb_dim, bias=True)
                                       for _ in range(6)
                                       ])
    ...
    def rel_aggr(self, graph, u_node, v_node, num_nodes, num_edges, aggr_flag, is_drop):
        ...
        if aggr_flag == 1:
            ...
            rel_neighbor_embd = sum([torch.sparse.mm(edge_connect_l[i],
                                                     self.fc_reld3[i](self.h2)) for i in range(self.link_mode)])
            ...
            rel_neighbor_embd = sum([torch.sparse.mm(edge_connect_l[i],
                                                     self.fc_reld2[i](self.h1)) for i in
    ...
    def forward(self, data):
        ...
        selected_edges = torch.LongTensor(random.sample(range(num_edges), int(num_edges*0.5)))

        selected_u_nodes = en_g.edges()[0][selected_edges]
        selected_v_nodes = en_g.edges()[1][selected_edges]
        ...
        '''
        h_0_N = self.rel_aggr(en_g, selected_u_nodes, selected_v_nodes, num_nodes, num_edges, aggr_flag=3, is_drop=True)

        self.h1 = self.h0.clone()
        for i, eid in enumerate(selected_edges):
            self.h1[eid] = self.h1[eid] + h_0_N[i]

        h_1_N = self.rel_aggr(en_g, neighbor_u_nodes, neighbor_v_nodes, num_nodes, num_edges, aggr_flag=2, is_drop=True)
        # print(h_0_N.shape)  # 18,32
        self.h2 = self.h1.clone()
        for i, eid in enumerate(neighbor_edges):
            self.h2[eid] = self.h2[eid] + h_1_N[i]

        rel_edge_ids = [en_g.edge_id(head_ids[i], tail_ids[i]) for i in range(head_ids.shape[0])]

        self.h2_extracted = self.h2[rel_edge_ids]
        self.rel_edge_ids = rel_edge_ids
        self.rel_edge_ids = rel_edge_ids
        h_2_N = self.rel_aggr(en_g, head_node, tail_node, num_nodes, num_edges, aggr_flag=1, is_drop=True)

        h3 = self.h2_extracted+h_2_N
